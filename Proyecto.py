...
selection = alt.selection_multi(fields=['comuna_cli', 'estado_civil'])
color = alt.condition(selection,
                      alt.Color('comuna_cli:N', legend=None),
                      alt.value('lightgray'))
...
legend = alt.Chart(data1).mark_rect().encode(
    y=alt.Y('comuna_cli:N', axis=alt.Axis(orient='right')),
#     x='estado_civil:N',
    color=color
).add_selection(
    selection
...
scatter = alt.Chart(data1).mark_circle().encode(
    x='m2:Q',
    y='uf_m2:Q',
    color=color,
#     tooltip='Name:N'
